...
poller" or an "APM" tool, so lets get some jargon straight before we move
ahead.

### Centralized pollers
The oldest tools we describe as "monitoring tools" are centralized pollers.
This is typically a single host that sits somewhere on the network and, like
the unfortunate employee in those cell-provider commercials, constantly asks
the question "can you hear me now?" to every host you configure it to
"monitor".  If any of those hosts fail to respond, the system sends you a
...
asked (usually via a listening network socket. Statsd is a wildly popular
...
way back to the Database (which, some say resides in Pittsburg).  Application
Performance Monitoring, or APM tries to measure the performance characteristics
of a web-application from one end to the other; breaking down how long every
little hunk of code took to do it's thing, so when someone says "the website is
slow" you can hopefully see where you need to go to fix it.
...
strenuously as the filed of IT monitoing. One of the seminal works on Data
Visualization is a book called "Visual Display of Quantitive Information" by
Edward Tufte. I highly recommend it. It positively brims with amazing advice,
all of which was completely lost on whomever designed many of the UI's in the
monitoringosphere.

This isn't the end of the world. I mean pretty graphics of questionable utility
have their place, but there's a line between decorating your creation and
creating decoration, and the tools that cross that line are communicating to
you that they care more about your bosses money than your visibility. So how do
you know when they've crossed the line? Well that's a little subjective, but
without reading Tufte yourself here are some Indicators:
...
Doughnut charts are pie-charts with a hole in the middle, so basically, they're
pie charts. Pie charts are not great because humans are notoriously bad at
interpreting the relative size of circular areas (really, we tend to
tremendously underestimate the difference between the areas of multiple
two-dimensional objects despite their shape). Whenever I see a pie-chart or
doughnut chart, it bothers me, because it's a sloppy means of depicting
information.  A bar-chart would have been the same size and far easier for me
to interpret.  Whenever I see one, I question why the designer who put a pie
chart here decided to do that.

Speedometers and stoplights take up a lot of space to depict not very much
information. Whenever I see them it bothers me because the designer could have
given me a small line-graph with some historical context in it, or a
large-number visualization which takes less cognative energy for me to
interpret. The thing to think about here is the data-to-ink ratio, how many
pixels is it taking the UI to depict this information to me. Is it showing me 1
value in a 150x150 pixel area (like most speedometers do), or is it showing me
300 values in that area? Is that single value 300 times more important than
what I could be seeing?  Sometimes it is and usually it isn't. If it is
300-times more important that I see that one value, then just show me the
value, don't show me a digital meter pointing at the value. If you want to
imply thresholds, then colorize the box the value is in or draw a bullet graph.
Literally the _only_ reason engineers used meters in the past was exactly
because they didn't have a means of just showing you the value, so using a
display that can clearly display the value to draw a meter pointing at a value
instead is kind of an abhorrant technological regression don't you think?

Node-Edge Diagrams and map overlays (where we depict hosts on a map of Belgium
for example) have not only terrible data-to-ink ratio, but they almost always
use up all that space to tell us what we already know. I don't pretend to know
the specifics of your current employment, but in my experience if I was working
somewhere and I didn't know where the servers were physically located, it
didn't matter where the servers were physically located. Likewise the only
node-edge diagrams that have ever been of any use to me were the ones that I
drew myself trying to understand the inner-workings of a new infrastructure.
There are certainly some APM tools I've seen that draw node-edge diagrams that
I can imagine being useful, but I remain skeptical about _how_ useful these
visualizations actually are in real life. Whenever I see them, it bothers me
and I find myself asking why they're there versus a collection of good-ole
well-chosen line-graphs. I might look at the map once or twice if ever, but if
the line graphs depict useful production metrics, I'll look at them every day.
